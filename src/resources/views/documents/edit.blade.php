...
// Multi-user recipient picker (pre-populated from existing document or old input)
@php
  $oldRecipientIds = array_map('intval', old('recipient_ids', $document->recipients->pluck('id')->all()));
  $oldExecutorIds = array_map('intval', old('executor_ids', $document->executors->pluck('id')->all()));
  if (empty($oldExecutorIds) && $document->executor_id) {
    $oldExecutorIds = [$document->executor_id];
  }
  ...
  $preExecutors = $users->whereIn('id', $oldExecutorIds)
    ->map(fn($u) => ['id' => $u->id, 'name' => $u->name])->values()->all();
  $preOrgs = old('recipient_orgs', $document->recipient_orgs ?? []);
@endphp
